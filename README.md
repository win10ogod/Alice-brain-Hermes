...
需求為 Python 3.11–3.13 與 Hermes Agent 0.18.x。建議直接以 Hermes 的 Git plugin
安裝器安裝這個獨立儲存庫：

```console
hermes plugins install win10ogod/Alice-brain-Hermes --enable
```

儲存庫根目錄的 `plugin.yaml` 與 `__init__.py` 是正式 directory-plugin 入口；
它直接載入同一 checkout 中的 `src/alice_brain_hermes`，不需要預先安裝本 package。
...
pip 安裝使用 `hermes_agent.plugins` entry point。Hermes 的第三方 plugin 預設不啟用；
若安裝時沒有使用 `--enable`，需明確啟用：

```console
hermes plugins enable alice-brain
```

以 Hermes 原生 plugin CLI 控制獨立 daemon（Git 與 pip 安裝都適用）。
所有命令都直接共用 Python handler，不以 subprocess 包裝另一個 CLI：
...
daemon 不由 session end/finalize/reset 停止，因此在無對話 turn 時仍能持續 C0；
只有明確的 `stop` 指令會停止它。

pip 安裝另外提供等價的獨立 `alice-brain-hermes` executable：
