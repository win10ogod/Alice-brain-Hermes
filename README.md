...
需求為 Python 3.11–3.13 與 Hermes Agent 0.18.2。請將 Hermes 與本 package
安裝到同一個 Python 環境，讓依賴與 `hermes_agent.plugins` entry point 一起安裝：
...
Hermes 的第三方 plugin 預設不啟用，因此安裝後必須明確啟用。

以 Hermes 原生 plugin CLI 控制獨立 daemon。
...
開發依賴固定 Hermes Agent 0.18.2，實際 host 合約與安裝 wheel 測試不會自動 skip。
